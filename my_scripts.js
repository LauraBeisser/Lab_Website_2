...
function loadStatsPage(){
	...
	for(rowCount = 2; rowCount < table.rows.length; rowCount++){
			oppTeam = table.rows[rowCount].cells[1].innerHTML;
			...
			if(homeScore > oppScore){
				table.rows[rowCount].cells[4].innerHTML = "CU";
				//document.getElementById(wins) = parseInt(cellValue) + 1;
				winner++;
			}
			else{
				table.rows[rowCount].cells[4].innerHTML = oppTeam;
				//document.getElementById(losses) = parseInt(cellValue) + 1;
				losses++;
			}
		}
...
function loadPlayersPage(){
	...
	for(i = 0; i < players.length; i++){
		namee = players[i].name;
		dropdownV += '<button onclick = "switchPlayers(' + i + ')">'  + namee + "</button>";
	}
	document.getElementById("player_selector").innerHTML = dropdownV;
}
...
function switchPlayers(playerNum){
	...
	avgRu = players[playerNum].rushing_yards / players[playerNum].games_played;
	avgRe = players[playerNum].receiving_yards / players[playerNum].games_played;
	document.getElementById("avg_p_yards").innerHTML = avgP.toFixed(2);
	document.getElementById("avg_r_yards").innerHTML =avgRu.toFixed(2);
	document.getElementById("avg_rec_yards").innerHTML = avgRe.toFixed(2);
}
